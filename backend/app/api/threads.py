...
def get_thread_messages(
    ...
    """Return chronological conversation history for an existing thread owned by user."""
    if crud.get_thread(db, thread_id, user_id=current_user.id) is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Thread not found."
        )
...
def get_study_log(
    ...
    """Return study activity for one existing thread owned by user."""
    if crud.get_thread(db, thread_id, user_id=current_user.id) is None:
...
def delete_thread(
    ...
    """Delete a thread owned by current user."""
    if crud.get_thread(db, thread_id, user_id=current_user.id) is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Thread not found.")
    try:
